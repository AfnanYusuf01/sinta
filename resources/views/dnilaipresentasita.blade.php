...
@section('title', 'Nilai Presentasi TA')

@section('page_title', 'Nilai Presentasi TA')
  ...
  <div class="col-md-3">
    <div class="card bg-primary text-white mb-4">
      <div class="card-body">
        ...
        <h2 class="mb-0">{{ $nilaiPresentasi->groupBy('id_mahasiswa')->count() }}</h2>
      </div>
    </div>
  </div>
  <div class="col-md-3">
    <div class="card bg-success text-white mb-4">
      <div class="card-body">
        ...
        <h2 class="mb-0">
          {{ $nilaiPresentasi->groupBy('id_mahasiswa')->filter(function($group) {
            $avgNilai = $group->avg(function($nilai) {
              return ($nilai->nilai_penyajian + $nilai->nilai_tingkat_penguasaan + 
  ...
  <div class="col-md-3">
    <div class="card bg-info text-white mb-4">
      <div class="card-body">
        ...
        <h2 class="mb-0">
          {{ number_format($nilaiPresentasi->avg(function($nilai) {
            return ($nilai->nilai_penyajian + $nilai->nilai_tingkat_penguasaan + 
                    $nilai->nilai_kualitas_jawaban + $nilai->nilai_sikap) / 4;
          }), 2) }}
        </h2>
...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <h2 class="mb-0">Nilai Presentasi TA</h2>
  ...
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-bordered table-hover" id="nilaiTable">
        ...
        <tbody>
          @foreach($nilaiPresentasi->groupBy('id_mahasiswa') as $idMahasiswa => $nilaiGroup)
          @php
              ...
              if ($nilaiP1) {
                  $nilaiP1Avg = ($nilaiP1->nilai_penyajian + $nilaiP1->nilai_tingkat_penguasaan + 
                                $nilaiP1->nilai_kualitas_jawaban + $nilaiP1->nilai_sikap) / 4;
                  $totalNilai += $nilaiP1Avg;
                  $pembimbingCount++;
              }
              
              if ($nilaiP2) {
                  $nilaiP2Avg = ($nilaiP2->nilai_penyajian + $nilaiP2->nilai_tingkat_penguasaan + 
                                $nilaiP2->nilai_kualitas_jawaban + $nilaiP2->nilai_sikap) / 4;
          ...
          <tr>
            ...
            <td class="text-center">
              <button class="btn btn-sm btn-danger" onclick="showNilaiPresentasi([
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title">Detail Nilai Presentasi TA</h5>
        ...
        <div class="card bg-light mt-4">
          <div class="card-body">
            <div class="row align-items-center">
              <div class="col-md-8">
                <h5 class="card-title mb-0">Nilai Akhir Presentasi</h5>
...
<script>
  ...
  async function showNilaiPresentasi(nilaiIds) {
    try {
      const responses = await Promise.all(nilaiIds.map(id => {
        if (id === null) return Promise.resolve(null);
        return fetch(`/admin/nilai-presentasi/${id}`).then(res => res.json());
      }));
      ...
      validResponses.forEach((nilai, index) => {
        const nilaiAvg = (nilai.nilai_penyajian + nilai.nilai_tingkat_penguasaan + 
                         nilai.nilai_kualitas_jawaban + nilai.nilai_sikap) / 4;
        totalNilai += nilaiAvg;
        pembimbingCount++;
          ...
          <div class="nilai-card-body">
            <div class="table-responsive">
              <table class="table table-sm table-bordered mb-0">
                <tbody>
                  <tr>
                    <td style="width: 60%">1. Penyajian Materi</td>
                    <td class="text-center">${nilai.nilai_penyajian}</td>
                  </tr>
                  <tr>
                    <td>2. Tingkat Penguasaan</td>
                    <td class="text-center">${nilai.nilai_tingkat_penguasaan}</td>
                  </tr>
                  <tr>
                    <td>3. Kualitas Jawaban</td>
                    <td class="text-center">${nilai.nilai_kualitas_jawaban}</td>
                  </tr>
                  <tr>
                    <td>4. Sikap dan Etika</td>
                    <td class="text-center">${nilai.nilai_sikap}</td>
                  </tr>
